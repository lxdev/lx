...
public class ClassUtil {
	 ...
	 * @return
	 */
	public static List<String> getClassNames(List<String> classList,
			String basePackpageName) {
//		Constants.PATH="/java/tomcat6/webapps/lx/WEB-INF/classes";
		String[] classStrs = getPackageAllClassName(Constants.PATH,
				basePackpageName);
		for (String pg : classStrs) {
			if (!pg.substring(0, 1).equals(pg.substring(0, 1).toUpperCase())) {
				...
				if (pg.indexOf(".class") > 0) {
					classList.add(basePackpageName + "."
							+ pg.substring(0, pg.indexOf(".class")));
				}
			}
		}
		return classList;
	...
	// int i=1;
	// for (String s : getClassNames("net.lx.entity")) {
