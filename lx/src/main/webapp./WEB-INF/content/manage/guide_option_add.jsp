...
<html>
  <head>
		...
		$(document).ready(function(){
				...
				$("#option_name").formValidator({onShow:"请输入选项名",onFocus:"选项名至少1个字符,最多20个字符",onCorrect:""}).inputValidator({min:1,max:20,onError:"你输入的选项名错误,请确认"});
			});
  ...
  <body>
		...
		<body:body>
			<%
				String actionUrl = basePath + "/manage/guide_option_add";
				if (!"0".equals(request.getAttribute("id")) && !request.getAttribute("id").equals(0)) {
					actionUrl = basePath + "/manage/guide_option_update";
				}
			%>
				<!--Search Begin-->
				<div>
					...
					<table width="100%" class="add_table" border="0" cellpadding="2" cellspacing="0">
						<tr>
							...
							<td><input type="text" class="txt_box_150" name="option_name" id="option_name" value="${option.option_name}"/></td>
							<td><div id="userNameTip" class="user_form_validator"></div></td>
						</tr>
						<tr>
							<td class="lable_100">父选项:</td>
							<td>
								<s:select list="%{parentList}" listKey="id" listValue="option_name" value="%{ option.parentOption == null ? 0 : option.parentOption.id }" disabled="%{ (option.parentOption == null && id != 0) ? true : false }" headerKey="0" headerValue="--请选择--" theme="simple" name="parent_id" id="parent_id" cssClass="select"/>
							</td>
						</tr>
						<tr>
							<td>&nbsp;</td>
