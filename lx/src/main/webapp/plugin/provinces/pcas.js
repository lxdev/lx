...
(function(){ // Global Settings BEGIN
	...
	/**
	 * 解析省份，如："安徽省$亳州市,瞧城区,涡阳县,蒙城县,利辛县|阜阳市,阜南区,凤台#河南省$商丘市,鹿邑"
	 * 其中，美元符($)以前的是省、直辖市的名称，美元符到下一个井号(#)之前的为此省下辖的市县(区)，各个地
	 * 级市之间用竖线(|)分割开
	 * 
	 * 返回结构：{ 安徽省:{
	 * 亳州市: [瞧城区,涡阳县,蒙城县,利辛县],
	 ...
	 * }
	 */
	function parseProvince(p_str)
	{
		var p_c= p_str.split('$');
		var P = {};
		var city={};
		var data_c_a = p_c[1].split('|');
		for(var j=0; j<data_c_a.length; j++){
			$.extend(city, parseCity(data_c_a[j]));
		}
	...
	var data_p_a = PCCC_DATA.split('#'); //拆开各省份数据
	
	for(var i=0; i<data_p_a.length; i++){
		$.extend(Q, parseProvince(data_p_a[i])); //解析各省份数据并添加到全局命名空间 Q 中
	}
	
})(); // Global Settings END

/* Function block */
(function($){ //jQueryPlugin Begin
...
$.PCCC = function(opts){
	var config = { // 默认设置
		...
		//countyV: '涡阳县',
		showHint: true,
		hint:[{0:'--请选择省份--'},{0:'--请选择城市--'},{0:'--请选择区县--'}]
	};
	
	$.extend(config, opts); //加载用户配置参数
	
	this.config = config;
	
	/** 注册省市级选择控件监听器 **/
	var _this = this;
	$(this.config.province).change(function(){ _this._province_listener(this); });
	$(this.config.city).change(function(){ _this._city_listener(this); });
	
	/** 初始化控件组，加载控件组默认值(用户指定) **/
	this._init();
};

/*
$.PCCC.prototype.config = {
	province:'#province',
	provinceV:'安徽省',
	city: '#city',
	cityV: '亳州市',
...
/** 初始化控件组，并加载控件组默认值(用户指定) **/
$.PCCC.prototype._init = function(){
	this._fill_province();
	
	$(this.config.province).val(this.config.provinceV).change();
	$(this.config.city).val(this.config.cityV).change();
	$(this.config.county).val(this.config.countyV);
};

/** 将省/直辖市填充的控件组中 **/
$.PCCC.prototype._fill_province=function(){
	$(this.config.province).empty();
	...
	if(hint){
		var opt = $('<option/>');
		opt.val(hint.key);
		opt.append(hint.value);
		opt.appendTo(this.config.province);
	}

	for(var p in Q){
		$('<option value="'+p+'">'+p+'</option>').appendTo(this.config.province);
	}
};

/** 地市级控件Change监听器，会根据showHint来决定是否显示提示项，已经当前选择提示项时下一级(县级)默认选中提示项(清除其他项) **/
$.PCCC.prototype._city_listener = function(){
	$(this.config.county).empty();
	
	var hint = this.hint(1);
	
	var countyHint = this.hint(2);
	if(countyHint){
		$('<option value="'+countyHint.key+'">'+countyHint.value+'</option>').appendTo(this.config.county);
	}
	
	var city = $(this.config.city).val();
	
	if(city != hint.key){
		var county = Q[$(this.config.province).val()][city];
		for(var i=0; county && i<county.length; i++){
			$('<option value="'+county[i]+'">'+county[i]+'</option>').appendTo(this.config.county);
		}
	}
};

/** 省级控件Change监听器，会根据showHint来决定是否显示提示项，已经当前选择提示项时下一级(地市级)默认选中提示项(清除其他项) **/
$.PCCC.prototype._province_listener = function(){
	$(this.config.city).empty(); //
	
	var hint = this.hint(0);
	
	var cityHint = this.hint(1);
	if(cityHint){
		$(this.config.city).prepend($('<option value="'+cityHint.key+'">'+cityHint.value+'</option>'));
	}

	var pv = $(this.config.province).val();
	
	if(pv != hint.key) {
		var city = Q[$(this.config.province).val()];
		for(var c in city){
			$('<option value="'+c+'">'+c+'</option>').appendTo(this.config.city);
		}
	}

	$(this.config.city).change(); //通知 地市级 控件，它的父级已经改变了
}

/** 获取用户设定的默认值： 0=>省 1=>市 2=>县 **/
$.PCCC.prototype.hint = function(index)
{
	if(!this.config.showHint || typeof(index)=='undefined' || index==null || isNaN(index) || index>this.config.hint.length)
		return false;
	
	var e = {}; //
	
	for(var name in this.config.hint[index]){
		e['key'] = name;
		break; //pick the first pair
	}
	
	e['value'] = this.config.hint[index][e.key];

	return e;
}

/** 按照默认值(用户配置参数)重置级联选择控件组 **/
$.PCCC.prototype.reset = function(){
	this._init();
};

/** 取得控件组的当前值，按省、市(、县，如果设置了区县选择控件)的顺序以数组的形式返回其值 **/
$.PCCC.prototype.value = function(){
	var v = [];
	v.push($(this.config.province).val());
	v.push($(this.config.city).val());
	if(this.config.county && $(this.config.county).size()==1){
		v.push($(this.config.county).val());
	}
